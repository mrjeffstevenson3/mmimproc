c gfortran  -O3 -mcmodel=medium -g readfiber_withchannel_ukf_fa1_injection.f -o readfiber_withchannel_ukf_fa1_injection -ffixed-line-length-none
...
c
	...
	real poly(9),dnmrsav(2000000,10), polysav(7000000,5,4),tensors(7000000,9)
...
c	write(6,*)'ufk ',cnmr(i),inmr(i),i
	...
	if(inmr(i).eq.10.and.i.gt.175)then
...
c	write(6,*)'ufk ',cnmr(ii),inmr(ii),ii
...
c save the indices for later tensors selection
		...
		enddo
c
c get 100 lines around this one line
c
		do iextra=1,100
		iexstart = i-50+iextra
...
c
	if(numpointsa.gt.100000.and.numpointsa.lt.1000000)then
	do i=1,43
	call fgetc(11,cnmr(i),istate)
	call fgetc(21,cnmr(i),istate)
...
c	if(i.le.ihalf)tensors(i,1)=0.5
c	if(i.gt.ihalf)tensors(i,1)=1.0
c
c this part below will repaint the tensor a different color if the fiber 
c went through the channel
c  if the fiber does not pass through the channel it will remain
...
c		write(6,*)'inside of fa1'
	...
	nmr(ii)= tensors(i,ii)
	enddo
	if(tensors(i,1).eq.2.0)write(6,*)'found fa1 1',i,ifinalcount
	incs = 1
	do ii=1,1        !within tensor

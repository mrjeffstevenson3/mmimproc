c gfortran  -O3 -mcmodel=medium -g writefiber_withpaint_with_multiplestats_may09_2017_ct.f -o writefiber_withpaint_with_multiplestats_may09_2017_ct -ffixed-line-length-none
...
c
	...
	character*1 vtk(80000000),c10(10),cspace(1),chead(348)
	equivalence (chead,ihead)
	equivalence (chead,rhead)
	integer*2 ihead(174)
	real rhead(87)
	integer*1 ivtk(80000000)
	...
	common /rot/rotation
	integer line(60000,9000),linesav(90000)
...
c	read(5,*)ibrainreg1,ibrainreg2,ibrainreg3
	...
	ibrainreg4 = 384
c
c now prepare the final stats file with overlap 
c
	do k=1,izsize
	do j=1,iysize
	do i=1,ixsize

	if(dti(i,j,k,1).eq.ibrainreg1.and.dti(i,j,k,2).ne.ibrainreg2.and.dti(i,j,k,3).ne.ibrainreg3)dti(i,j,k,6)=1.0
	if(dti(i,j,k,1).ne.ibrainreg1.and.dti(i,j,k,2).eq.ibrainreg2.and.dti(i,j,k,3).ne.ibrainreg3)dti(i,j,k,6)=2.0
	if(dti(i,j,k,1).ne.ibrainreg1.and.dti(i,j,k,2).ne.ibrainreg2.and.dti(i,j,k,3).eq.ibrainreg3)dti(i,j,k,6)=3.0
	if(dti(i,j,k,4).eq.ibrainreg4)dti(i,j,k,6)=4.0
	if(dti(i,j,k,5).eq.11169.0)dti(i,j,k,6)=5.0
	if(dti(i,j,k,1).eq.ibrainreg1.and.dti(i,j,k,4).eq.ibrainreg4)dti(i,j,k,6)=6.0
	enddo
	enddo
	enddo
c
c test image for dti 6
c
	open(11,file = 'test6.img',form='unformatted')

	do k=1,izsize
	do j=1,iysize
	do i=1,ixsize
	dtistats(i) = dti(i,j,k,6)
	enddo  !i

	do i=1,ixsize*4
	call fputc(11,cdti(i),istate)
	enddo
	enddo  !j
	enddo  !k
	close(11)
c
	ihead(25) = 1
	open(11,file = 'test6.hdr',form='unformatted')
	do i=1,348
	call fputc(11,chead(i),istate)
	enddo
	close(11)
c
...
c
	...
	do i=1,10
	write(6,*)tensors(i,1),i
	enddo
	do i=itensors-100,itensors
	write(6,*)tensors(i,1),i
	enddo
...
c	tensors(i,1) = ifil+1
	...
	enddo  !itensors
	do ifil=1,7
	write(6,*)'iflagindex ',iflagindex(ifil),ifil
	enddo

c gfortran  -O3 -mcmodel=medium -g writefiber_withpaint_with_multiplestats_channelized_050417.f -o writefiber_withpaint_with_multiplestats_channelized_050417 -ffixed-line-length-none
...
c	write(6,*)'found stats ',iix,iiy,iiz
	...
	enddo
c set all tensors to 0 outside the channel
c
c
	inc1 = 0
	inc2 = 0
	...
	iflag = 0
		do ii=1,ifinalcount
		ipoly = polyfinal(ii,1)
		if(i.eq.ipoly)iflag = 1
		enddo  !ii
	if(iflag.eq.0)tensors(i,1) = 0.01
	if(iflag.eq.0)inc1 = inc1+1
	if(iflag.eq.1)inc2 = inc2+1
	enddo  !i
	write(6,*)'there are outside channel and inside channel ',inc1,inc2

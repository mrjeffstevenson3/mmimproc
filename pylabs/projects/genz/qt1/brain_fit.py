...
b1map, spgr05, spgr10, spgr15, spgr20, spgr30 = b1map_fnames[0], spgr5_fa5_fnames[0], spgr5_fa10_fnames[0], spgr5_fa15_fnames[0], spgr5_fa20_fnames[0], spgr5_fa30_fnames[0]

#for b1map, spgr05, spgr15, spgr30 in zip(b1map_fnames, spgr_fa5_fnames, spgr_fa15_fnames, spgr_fa30_fnames):

# inside for loop:
# thr spgr30 @ 2000
# then loop over other
...
for b1map, spgr05, spgr10, spgr15, spgr20, spgr30 in zip(b1map_fnames, spgr5_fa5_fnames, spgr5_fa10_fnames, spgr5_fa15_fnames, spgr5_fa20_fnames, spgr5_fa30_fnames):
    ...
    if not (b1map_dir/str(b1map+'_mag'+ext)).is_file():
        b1magcmd = ['fslroi '+str(b1map_dir/b1map+'.nii')+' '+str(replacesuffix(b1map_dir/b1map, '_mag'+ext))+' 0 1']
...
for b1map, spgr05, spgr10, spgr15, spgr20, spgr30 in zip(b1map_fnames, spgr5_fa5_fnames, spgr5_fa10_fnames, spgr5_fa15_fnames, spgr5_fa20_fnames, spgr5_fa30_fnames):
    ...
    b1tospgr30antscmd = [ str(antsRegistrationSyNQuick), '-d 3 -m', str(appendposix(b1map_dir/b1map, '_mag.nii.gz')), '-f',
                str(spgr_dir/str(spgr30+'.nii')), '-o', str(appendposix(b1map_dir/b1map, '_mag_reg2spgr30_')),
                '-n 30 -t s -p f -j 1 -s 10 -r 1']
    spgr05tospgr30antscmd = [ str(antsRegistrationSyNQuick), '-d 3 -m', str(spgr_dir/str(spgr05+'.nii')), '-f',
                str(spgr_dir/str(spgr30+'.nii')), '-o', str(spgr_dir/str(spgr05 + '_reg2spgr30_')),
                '-n 30 -t s -p f -j 1 -s 10 -r 1']
    spgr15tospgr30antscmd = [ str(antsRegistrationSyNQuick), '-d 3 -m', str(spgr_dir/str(spgr15+'.nii')), '-f',
                str(spgr_dir/str(spgr30+'.nii')), '-o', str(spgr_dir/str(spgr15 + '_reg2spgr30_')),
                '-n 30 -t s -p f -j 1 -s 10 -r 1']
    if flip5:
        spgr10tospgr30antscmd = [str(antsRegistrationSyNQuick), '-d 3 -m', str(spgr_dir / str(spgr10 + '.nii')), '-f',
                                 str(spgr_dir / str(spgr30 + '.nii')), '-o',
                                 str(spgr_dir / str(spgr10 + '_reg2spgr30_')),
                                 '-n 30 -t s -p f -j 1 -s 10 -r 1']
        spgr20tospgr30antscmd = [str(antsRegistrationSyNQuick), '-d 3 -m', str(spgr_dir / str(spgr20 + '.nii')), '-f',
                                 str(spgr_dir / str(spgr30 + '.nii')), '-o',

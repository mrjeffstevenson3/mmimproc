...
spm_defaults; global defaults; params = defaults.normalise.estimate;       % OLD
%spm_defaults; global defaults; params = defaults.old.normalise.estimate;  % NEW from DJP
...
for(ii=1:sz(4))
    ...
    for(jj=1:numel(fg.fibers))
        fp(ii,jj) = nanmean(p([fiberCoord:fiberCoord+fgLen(jj)-1])'); % transpose added by djp

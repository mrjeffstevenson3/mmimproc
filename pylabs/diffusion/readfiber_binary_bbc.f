...
c
	...
	real psavx(3),psavy(3),psavz(3),rmindistance(3),imindistance1(3),imindistance2(3)
c
...
c
	...
	enddo
	if(i.eq.112)then
	write(6,*)'line ',line(i,1),(line(i,ii),ii=2,line(i,1)+1)
...
c	write(6,*)'this is indexing for the last line '
c	write(6,*)line(ilines,1),(line(ilines,ii),ii=2,line(ilines,1)+1)
...
c	write(6,*)'the minumum z and index for line ',i,rminz,iminz
	write(6,*)'the minimum distance between line and model is ',rmindistance(ifil),imindistance1(ifil),imindistance2(ifil),ifil,i
	enddo
	
c  for the rmaxz find the closest cortex model point
c

	if(rmindistance(2).lt.2.0.and.rmindistance(3).le.2.0)then
	igood = igood+1
	dnmr(igood) = sum   ! sum distance for one line
c save the indices for later tensors selection
		do ii=2,line(i,1)+1,3
		polyfinal(ifinalinc,1) = line(i,ii)+1
	write(6,*)'polyfinal ',polyfinal(ifinalinc,1),ifinalinc,ii,i
...
c	write(6,*)'distance ',dnmr(i),i,ipsize
	...
	write(23,*)averlength
	close(23)
	write(6,*)'the number of fibers is ', ilines,incpass_size
...
c	write(6,*)'tensor ',nmr(ii),ii
	...
	if(i.eq.16)write(6,*)'found evil tensor ',i
c	pause
	enddo  !ilines
	do ii=1,9
	write(6,*)'tensor ',tensors(16,ii),i
	enddo
c
c test output 
	do ii=1,9
...
c
	...
	inc = inc+1
	write(6,*)'rfa1 axial radial rmd volumeratio ',r1,r2,r3,rfa1,axial,radial,rmd,volumeratio,i
...
c		write(6,*)'dnmr(inc2) ',dnmr(inc2),inc2

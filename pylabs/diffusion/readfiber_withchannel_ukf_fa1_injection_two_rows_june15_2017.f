...
c
	...
	common /rot/rotation
	integer line(600000,900),linesav(90000)
...
c
	open(11,file = 'mori.hdr',form='unformatted')
	do i=1,348
	...
	rzdim = rhead(23)
c
	open(11,file = 'mori.img',form='unformatted')
	...
	close(11)
c
c read in the config file that has the brain regions for boundaries
c
	open(11,file = 'config.txt')
	read(11,*)c37
	read(11,*)istartrow1,iendrow1
	read(11,*)istartrow2,iendrow2
	read(11,*)isuperior1,isuperior2
	close(11)
c
c find the most posterior y coordinate for istartrow1 and istartrow2
c find the most anterior y coorindate for iendrow1 and iendrow2
c
	iymin1=100000
	iymin2=100000
	iymax1=0
	iymax2=0
c
c find the z coordinates of the places where the two isuperior regions meet
c
	inc = 1
	do k=1,izsize
	do j=1,iysize
	do i=1,ixsize
	if(dti(i,j,k,1).eq.istartrow1)iymin1=min(j,iymin1)
	if(dti(i,j,k,1).eq.istartrow2)iymin2=min(j,iymin2)
	if(dti(i,j,k,1).eq.iendrow1)iymax1=max(j,iymax1)
	if(dti(i,j,k,1).eq.iendrow2)iymax2=max(j,iymax2)
	if(dti(i,j,k,1).eq.isuperior1.and.dti(i,j,k+1,1).eq.isuperior2)then
	dnmr(inc) = k
	inc = inc+1
	endif
	...
	enddo  !j
	enddo  !k
	write(6,*)'results of the mori atlas boundaries'
	write(6,*)iymin1,iymin2,iymax1,iymax2,dnmr(1),inc-1
c find the average for superior boundary
c
	sum = 0
	do i=1,inc-1
	sum = sum+dnmr(i)
...
c
	...
	slicerstartrow1=(iymin1-yoffset)*rydim
	slicerendrow1=(iymax1-yoffset)*rydim
	slicerstartrow2=(iymin2-yoffset)*rydim
	slicerendrow2=(iymax2-yoffset)*rydim
	slicersuperior=(aversup-zoffset)*rzdim
	write(6,*)'slicerstart ',slicerstartrow1,slicerendrow1,slicerstartrow2,slicerendrow2,slicersuperior
...
c
	...
	rend = rstart+rfact
	write(6,*)'top row ',rstart,rend,ifil
	do i=1,itensors
		do ii=1,ifinalcount
		ipoly = polyfinal(ii,1)
		polyy = polysav(i,2,1)
...
c
	...
	rstart = rminy+ ((r1-1)*rfact)
	rend = rstart+rfact
	write(6,*)'bottom row ',rstart,rend,ifil
	do i=1,itensors
		do ii=1,ifinalcount
		ipoly = polyfinal(ii,1)
		polyy = polysav(i,2,1)

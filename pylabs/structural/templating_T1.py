...
for subj in subjects:
    for ses in [1, 2]:    # arbitrary! fix by testing range in dict
        for run in [1, 2, 3, 4]:      # arbitrary! fix by testing range in dict
            ...
            res = flt.run()
            cmd = 'fslroi '+niftiDict[k1b][k2b]['outfilename']+' '+niftiDict[k1b][k2b]['outpath']+'/'+k2b+'_phase 2 1'
            subprocess.check_call(cmd, shell=True)
            ...
            cmd += niftiDict[k1b][k2b]['outpath']+'/'+k2b+'_phase_reg2wempr.nii.gz'
            subprocess.check_call(cmd, shell=True)
            niftiDict[k1b][k2b]['phase_reg2vbm_s6_fname'] = niftiDict[k1b][k2b]['outpath']+'/'+k2b+'_phase_reg2vbmmpr.nii.gz'
            cmd = 'fslmaths '+niftiDict[k1a][k2v]['rms_fname']+' -div '+niftiDict[k1b][k2b]['phase_reg2vbm_s6_fname']
            cmd += ' -mul 100 '+niftiDict[k1a][k2v]['outpath']+ '/' + k2v + '_rms_b1corr.nii.gz'
            subprocess.check_call(cmd, shell=True)
            niftiDict[k1a][k2v]['rms_b1corr_fname'] = niftiDict[k1a][k2v]['outpath']+ '/' + k2v + '_rms_b1corr.nii.gz'
            cmd = 'fslmaths ' + niftiDict[k1a][k2w]['rms_fname'] + ' -div ' + niftiDict[k1a][k2b]['phase_reg2vbm_s6_fname']
            cmd += ' -mul 100 ' + niftiDict[k1a][k2w]['outpath'] + '/' + k2w + '_rms_b1corr.nii.gz'
            subprocess.check_call(cmd, shell=True)
            niftiDict[k1a][k2w]['rms_b1corr_fname'] = niftiDict[k1a][k2w]['outpath'] + '/' + k2w + '_rms_b1corr.nii.gz'
